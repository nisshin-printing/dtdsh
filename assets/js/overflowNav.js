...
(function(factory) {
		...
		var okayNav = 'okayNav',
				defaults = {
						...
						toggle_icon_class: 'overflowNav--menu__toggle',
		...
		$.extend(Plugin.prototype, {
				...
				init: function() {
						...
						$('body').addClass('overflowNav--loaded');

						// Add classes
						self.navigation.addClass('overflowNav loaded')
								.children('ul').addClass('overflowNav--nav__visible');

						// Append elements
						if (self.options.align_right) {
								self.navigation.append('<ul class="overflowNav--nav__invisible transition-enabled nav-right" />')
										.append('<a class="' + self.options.toggle_icon_class + ' overflowNav--invisible">' + self.options.toggle_icon_content + '</a>')
						} else {
								self.navigation.prepend('<ul class="overflowNav--nav__invisible transition-enabled nav-left" />')
										.prepend('<a class="' + self.options.toggle_icon_class + ' overflowNav--invisible">' + self.options.toggle_icon_content + '</a>')
						}

						// Cache new elements for further use
						self.nav_visible = self.navigation.children('.overflowNav--nav__visible');
						self.nav_invisible = self.navigation.children('.overflowNav--nav__invisible');
				...
				initEvents: function() {
						...
						self.document.on('click.okayNav', function(e) {
								...
								if (self.nav_open === true && _target.closest('.overflowNav').length == 0) self.closeInvisibleNav();
				...
				initSwipeEvents: function() {
						var self = this;
						self.document.on('touchstart.okayNav', function(e) {
										...
										if (e.originalEvent.touches.length == 1) {
												...
												if (
														((touch.pageX < 25 && self.options.align_right == false) || (touch.pageX > ($(self.options.parent).outerWidth(true) - 25) && self.options.align_right == true)) || self.nav_open === true) {
				...
				openInvisibleNav: function() {
						...
						self.toggle_icon.addClass('is-active');
						self.nav_invisible.addClass('has-nav-open');
						self.nav_open = true;
				...
				closeInvisibleNav: function() {
						...
						self.toggle_icon.removeClass('is-active');
						self.nav_invisible.removeClass('has-nav-open');
				...
				recalcNav: function() {
						...
						if (wrapper_width > expandAll_width) {
								...
								self.toggle_icon.addClass('overflowNav--invisible');
								return;
						}

						if (visible_nav_items > 0 && nav_full_width <= collapse_width && wrapper_width <= expand_width) {
						...
						self.getHiddenItemCount() == 0 ? self.toggle_icon.addClass('overflowNav--invisible') : self.toggle_icon.removeClass('overflowNav--invisible');
				},
				...
				destroy: function() {
						...
						self.nav_visible.removeClass('overflowNav--nav__visible');
						self.toggle_icon.remove();

						self.document.unbind('.overflowNav');
						self.window.unbind('.overflowNav');
				}
